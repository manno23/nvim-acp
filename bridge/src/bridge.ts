...
import type { RpcStub } from "./capnweb-shim.js";
import {
  Acp,
  ActionEvent,
  ActionRequest,
...
function logInvariant(event: string, details: Record<string, unknown>) {
  process.stderr.write(`[acp-bridge:${event}] ${JSON.stringify(details)}\n`);
}

class BridgeJobObserver extends RpcTarget implements JobObserver {
  constructor(
    private readonly jobId: string,
    private readonly writer: StdoutWriter,
    private readonly onResult: (jobId: string) => void,
  ) {
    super();
  }

  async event(event: ActionEvent): Promise<void> {
  ...
  async result(result: ActionResult): Promise<void> {
...
class BridgeFactsObserver extends RpcTarget implements FactsObserver {
  constructor(
    private readonly subscriptionId: string,
    private readonly writer: StdoutWriter,
    private readonly onClosed: (subscriptionId: string) => void,
  ) {
    super();
  }

  async fact(record: FactPayload["fact"]): Promise<void> {
  ...
  async closed(reason?: string): Promise<void> {
    logInvariant("fact_closed", { subscriptionId: this.subscriptionId, reason });
    this.onClosed(this.subscriptionId);
  }
}

type JobStub = Awaited<ReturnType<Acp["submit"]>>;
type FactsStub = Awaited<ReturnType<Acp["subscribeFacts"]>>;

class AcpConnection {
  private socket?: WebSocket;
  private stub?: RpcStub<Acp>;
  private readonly jobs = new Map<string, { stub: JobStub; observer: BridgeJobObserver }>();
  private readonly subscriptions = new Map<string, { stub: FactsStub; observer: BridgeFactsObserver }>();
  ...
  async connect(config: BridgeConfig): Promise<void> {
    ...
    this.socket = socket;
    this.stub = newWebSocketRpcSession<Acp>(socket as any);
  ...
  async closeFacts(subscriptionId: string) {
    this.assertConnected();
  ...
  shutdown() {
    ...
    if (this.stub && typeof (this.stub as any)[Symbol.dispose] === "function") {
      (this.stub as any)[Symbol.dispose]();
    }
    this.stub = undefined;
  }

  private async awaitOpen(socket: WebSocket, timeout: number) {
  ...
  private finishJob(jobId: string) {
    ...
    if (typeof (handle.stub as any)[Symbol.dispose] === "function") {
      (handle.stub as any)[Symbol.dispose]();
    }
  }

  private finishSubscription(subscriptionId: string) {
    const handle = this.subscriptions.get(subscriptionId);
    if (!handle) {
      return;
    }
